...
async function testOpenAIGeminiFallback() {
  console.log('🧪 Testing OpenAI → Gemini Fallback System...\n');
  ...
  try {
    console.log('\n📝 Testing OpenAI Summarization with Gemini Fallback...');
    console.log('💡 If OpenAI fails, it will automatically fallback to Gemini');
    ...
    // Check which service was used (this will be logged during execution)
    console.log('\n🔍 Service Used: Check the logs above to see which service was used');
    
  } catch (error) {
    console.error('\n❌ All summarization services failed:', error.message);
    
    if (error.message.includes('quota')) {
      console.log('\n💡 This might be a quota issue. Check your OpenAI dashboard.');
    } else if (error.message.includes('authentication')) {
      console.log('\n💡 This might be an API key issue. Check your API keys.');
    } else if (error.message.includes('Gemini')) {
      console.log('\n💡 Gemini API might not be enabled. Check your Google Cloud project.');
    }
  }
}

// Run the test
testOpenAIGeminiFallback().catch(console.error);
